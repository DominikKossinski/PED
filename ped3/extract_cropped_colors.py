...
from imgprocessing.colors_extractor import get_cropped_image_color, get_basic_colors_array, get_basic_colors
...
def run_colors(df: pd.DataFrame, export_path: str):
    ...
    for _, row in tqdm(df.iterrows()):
        ...
        if not error:
            max_colors = get_cropped_image_color(path, get_basic_colors_array(), get_basic_colors())
...
if __name__ == '__main__':
    ...
    full_path = os.path.join("..", "cropped_colors", size)
