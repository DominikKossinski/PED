...
def process_dir(dir_name: str, save_dir_name: str):
    ...
    for name in attrs:
        gb_data[name] = gb_data[name].apply(lambda x: eval(x)[-1] if eval(x) else nan)
        us_data[name] = us_data[name].apply(lambda x: eval(x)[-1] if eval(x) else nan)
    ...
    save_csv("ped5_trending_original", [gb_data, us_data], ["GB_videos", "US_videos"])


def main():
    process_dir("ped5_trending_original", "ped5_trending_original")
